 ...
 *
 * 🔴 **왜 게이트 훅을 그대로 쓰나** — 재려는 것이 *"자소서 AI 를 쓸 수 있는 환경이었는가"* 이고,
 * 그 판정은 이미 `useCoverletterAiBlocked` 가 갖고 있다. 같은 조건을 다시 구현하면
 * (뷰포트 헤더·UA 파싱 등) **게이트와 신호가 어긋나는 순간 분모가 거짓말을 시작한다.**
 * 여기서는 게이트의 부정(`aiBlocked === false`)이 곧 신호라 어긋날 수가 없다.
 * (문항·답변 편집은 2026-08-23 부터 모바일에서도 가능하다 — 이 신호가 재는 건 **AI 환경**이다.)
 *
 ...
 * ## 발사 조건
 *
 * - `enabled` (데모·비로그인 제외) **그리고** `aiBlocked === false`
