            ...
            <div role="tablist" aria-label="지원 상태 필터" className="flex items-center gap-1.5 sm:gap-1 whitespace-nowrap sm:bg-surface-2 sm:border sm:border-line sm:rounded-lg sm:p-1">
              ...
              return (
                ...
                >
                  {tab.label}
                  {count > 0 && (
                    <span className={`ml-1.5 text-[10px] ${isActive ? 'text-brand sm:text-text-tertiary' : 'text-text-quaternary'}`}>
                      {count}
                    </span>
                  )}
                </button>
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      {filter === 'all' && (
        <button
          onClick={onAdd}
          className="bg-brand hover:bg-accent active:bg-accent-hover text-bg text-xs font-semibold px-4 py-2.5 rounded-lg transition-colors shadow-lg shadow-brand/20"
        >
          + 첫 회사 추가하기
        </button>
      )}
