...
export function TermsAgreement() {
  ...
  const handleSubmit = async () => {
    ...
    // W1 — onboardedAt null (신규 user) → /signup/question. 기존 user → /calendar (홈)
    // 캘린더 UX 재구성 — 홈 = /calendar · /dashboard 는 '회고' 페이지로 강등
    if (!user?.onboardedAt) {
            ...
            {expandedTerms && (
              <div id="terms-content" className="px-5 pb-4">
                <div className="bg-bg rounded-lg p-4 text-text-quaternary text-xs leading-relaxed max-h-40 overflow-y-auto">
                  ...
                  <br />
                  <p>제3조 (서비스 이용) 서비스는 소셜 계정(카카오 또는 Apple)을 통한 로그인으로 이용할 수 있습니다. 서비스를 통해 입력한 정보는 회원의 자산으로 보호됩니다.</p>
                  <br />
            ...
            {expandedPrivacy && (
              <div id="privacy-content" className="px-5 pb-4">
                <div className="bg-bg rounded-lg p-4 text-text-quaternary text-xs leading-relaxed max-h-40 overflow-y-auto">
                  ...
                  <br />
                  <p>수집 항목: 소셜 계정 식별자(카카오·Apple), 닉네임, 이름(Apple), 이메일(선택 — Apple '이메일 가리기' 시 중계 주소)</p>
                  <br />
        ...
        <button
          ...
          className={`w-full py-3.5 rounded-xl text-sm font-semibold transition-all
            ${allAgreed && !submitting
              ? 'bg-brand hover:bg-accent active:bg-accent-hover text-bg shadow-[0_0_20px_rgb(var(--brand)/0.3)]'
