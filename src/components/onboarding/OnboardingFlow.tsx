  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/60 backdrop-blur-sm pb-[calc(env(safe-area-inset-bottom)+4rem)] lg:pb-0">
        <div role="dialog" aria-modal="true" aria-label="치뽀 온보딩" className="w-full md:max-w-[420px] bg-surface border border-line[0.06] rounded-t-2xl md:rounded-2xl shadow-2xl">
            ...
            {([1, 2, 3] as const).map((i) => (
              <span
                ...
                className={`h-1.5 rounded-full transition-all duration-300 ${
                  i === step ? 'bg-brand w-4' : 'bg-card w-1.5'
                }`}
              />
            ))}
          </div>
            ...
            <div className="px-6 pt-6 pb-4">
              ...
              <div className="grid grid-cols-3 gap-2.5">
                {FEATURES.map(({ icon, label, desc }) => (
                  <div
                    key={label}
                    className="flex flex-col items-center gap-2 rounded-xl p-3 bg-card border border-line[0.06]"
                  >
